...
module.exports = function () {
  ...
  return function inferMembership(comment) {
    ...
    if (path.isObjectProperty() && (path.get('key').isIdentifier() || path.get('key').isLiteral())) {
      path = path.get('key');
    }
    ...
    if (path.isMemberExpression()) {
      const memberIdentifiers = [].concat(extractThis(path, comment), extractIdentifiers(path));
    ...
    if ((path.isClassMethod() || path.isClassProperty()) && path.parentPath.isClassBody() && path.parentPath.parentPath.isClass()) {
      ...
      return inferMembershipFromIdentifiers(comment, inferClassMembership(path.parentPath.parentPath), scope);
    }
    ...
    if ((path.isIdentifier() || path.isLiteral()) && path.parentPath.isObjectProperty() && path.parentPath.parentPath.isObjectExpression()) {
    ...
    if (objectParent) {
      ...
      const lendsIdentifiers = findLendsIdentifiers(objectParent) || findLendsIdentifiers(objectParent.get('properties')[0]);
