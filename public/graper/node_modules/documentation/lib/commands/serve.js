  ...
  getPort({ port: argv.port }).then(port => {
    ...
    function updateWatcher() {
      ...
      documentation.expandInputs(argv.input, argv).then(files => {
        watcher.add(files.map(data => typeof data === 'string' ? data : data.file));
      }).catch(err => {
        /* eslint no-console: 0 */
    ...
    function updateServer() {
      documentation.build(argv.input, argv).then(comments => documentation.formats.html(comments, argv)).then(files => {
        if (argv.watch) {
          updateWatcher();
        }
